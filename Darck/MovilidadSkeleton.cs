...
public class MovilidadSkeleton : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isDead && !isWaiting) // Solo se mueve si no está muerto y no está esperando
        {
            ...
            {
                animator.SetBool("Caminar", false); // Asegurarse de detener la animación al girar
                Flip();
            }
        }
        else
    ...
    void Patrol()
    {
        ...
        // Solo activa la animación de caminar si el enemigo realmente se está moviendo
        ...
        {
            StartCoroutine(WaitAtWaypoint()); // Iniciar la espera si ya llegó al waypoint
        }
    }
    ...
    IEnumerator WaitAtWaypoint()
    {
        ...
        animator.SetBool("Caminar", false); // Detener animación al esperar
    ...
    public void SetDead(bool deadStatus)
    {
        ...
        if (isDead)
        {
            animator.SetBool("Caminar", false); // Detener la animación si está muerto
        }
    }
}
