...
public class AdminMySQL : MonoBehaviour
{
    ...
    public bool ReduceScore(int userId, int scoreToSubtract)
    {
        string query = "UPDATE users SET Score = Score - @score WHERE id = @userId AND Score >= @score";

        using (MySqlCommand cmd = connection.CreateCommand())
        {
            cmd.CommandText = query;
            cmd.Parameters.AddWithValue("@score", scoreToSubtract);
            cmd.Parameters.AddWithValue("@userId", userId);
